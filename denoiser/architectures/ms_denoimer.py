...
class SpatialandChannelAttentionBlock(nn.Module):
    def __init__(
            self,
            dim,
            window_size=(8, 8),
    ...
    def forward(self, x):
        for (spatial_attn, ff1, channel_attn, ff2) in self.blocks:
            x = spatial_attn(x) + x
            x = ff1(x) + x
            x = channel_attn(x) + x
            x = ff2(x) + x
...
class Denoimer(nn.Module):
    def __init__(self,
        ...
        for i in range(num_level-1):
            self.Encoder.append(
                SpatialandChannelAttentionBlock(
                    dim = dim * 2 ** i, 
        ...
        self.BottleNeck = SpatialandChannelAttentionBlock(
...
class MultiStageDenoimer(BaseModel):
    def __init__(self,
                 stages,
                 num_level,
                 share_params,
        ...
        ]) if not share_params else Denoimer(
                in_dim = in_dim,
                dim = dim, 
                out_dim = out_dim,
                window_sizes = window_sizes,
