...
class ChannelAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        for (channel_attn, ff) in self.blocks:
            x = channel_attn(x) + x
            x = ff(x) + x
        return x
...
class Restormer(nn.Module):
    def __init__(self,
        ...
        self.BottleNeck = ChannelAttentionBlock(
...
class MultiStageRestormer(BaseModel):
    def __init__(self,
                 stages,
                 num_level,
                 share_params,
        ...
        ]) if not share_params else Restormer(
                in_dim = in_dim,
                dim = dim, 
                out_dim = out_dim,
                layernorm_type = layernorm_type,
...
class MultiStageRestormerProfiling(BaseModel):
    def __init__(self,
                 stages,
                 share_params,
                 in_dim,
